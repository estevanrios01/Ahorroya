...
export function csrfProtection(request) {
  ...
  try {
    ...
    // originUrl.host === host already covers every legitimate same-origin
    // call, including this project's own preview deployments (a request
    // made from ahorroya-<hash>.vercel.app to itself has Origin === Host).
    // A separate "any hostname ending in .vercel.app" allowance let a
    // request forged from literally any other Vercel-hosted site --
    // trivial and free for anyone to stand up -- pass this check, which
    // defeated the CSRF protection for no legitimate reason.
