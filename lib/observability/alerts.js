...
const alertRules = [
  // getMetrics() names this field lastStatus, not status -- this comparison
  // never matched anything, so the rule could never fire regardless of
  // real scraper health.
  { name: 'scraper_down', check: (m) => { const down = Object.entries(m.scrapers).filter(([, s]) => s.lastStatus === 'error'); return down.length > 0 ? { level: 'critical', detail: `Scrapers caídos: ${down.map(([k]) => k).join(', ')}` } : null; } },
