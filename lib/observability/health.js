...
async function checkSupabase() {
  ...
  try {
    ...
    const { error } = await withTimeout(supabase.from('master_products').select('id', { count: 'planned', head: true }).limit(1), 1500, 'health timeout');
...
async function checkDatabase() {
  ...
  const start = Date.now();
  try {
    const { error } = await withTimeout(supabaseAdmin.from('stores').select('id', { count: 'planned', head: true }).limit(1), 1500, 'health timeout');
    const latency = Date.now() - start;
...
async function checkScrapers() {
  try {
    ...
    for (const store of ['exito', 'd1', 'jumbo', 'ara', 'carulla', 'olimpica', 'makro', 'farmatodo', 'cruz-verde', 'larebaja']) {
