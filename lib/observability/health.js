...
async function checkSupabase() {
  ...
  try {
    const { error } = await supabase.from('master_products').select('id', { count: 'exact', head: true }).limit(1);
...
async function checkDatabase() {
  ...
  const start = Date.now();
  try {
    const { error } = await supabaseAdmin.from('_schema').select('*', { count: 'exact', head: true }).limit(1);
    const latency = Date.now() - start;
...
async function checkScrapers() {
  try {
    const { data } = await supabase.from('scraping_jobs').select('store, status, updated_at').order('created_at', { ascending: false }).limit(50);
    const storeStatus = {};
