...
| M12 | Stores list duplicada | Resuelto ahora que hay datos reales para anclar la decisión: sembré la tabla `stores` (26 tiendas reales, metadata sacada literal de los scripts de importación) y `categories` (8) directo por SQL vía el conector de Supabase. "La Rebaja" quedó con slug `larebaja` (sin guion, el que realmente usa `scripts/import-vtex-catalog.js`). Unifiqué los 8 archivos que usaban `la-rebaja` (con guion) para que coincidan: `fallbackCatalog.js`, `liveFallbackProducts.js` (SOURCES + `PHARMACY_SOURCE_SLUGS`, incluido remover los 2 shims de alias que ya no hacen falta), `sitemap.js`, `StoreClient.jsx`, `health.js`, `actualizador-precios.js` y `ProductDetailClient.jsx`. `services/scrapers/config.js` quedó con `la-rebaja` a propósito -- es un namespace interno de jobs que nunca escribe a la tabla `stores`, no hay riesgo de choque. | ✅ FIXED (2026-07-26) |
