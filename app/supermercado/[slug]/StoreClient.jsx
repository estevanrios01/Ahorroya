...
export default function StoreClient({ store, products: initialProducts, totalProducts = 0, degraded = false }) {
  const [page, setPage] = useState(1);
  const [allProducts, setAllProducts] = useState(() => initialProducts || []);
  const [totalCount, setTotalCount] = useState(totalProducts);
  const [loading, setLoading] = useState(() => !(initialProducts || []).length);
  const [canLoadMore, setCanLoadMore] = useState(() => degraded && (initialProducts || []).length >= 24);
  const hasMore = canLoadMore || (totalCount > 0 && allProducts.length < totalCount);
    ...
    async function loadFallbackProducts() {
      ...
      try {
        ...
        if (!cancelled && json.success) {
          ...
          setCanLoadMore(Boolean(json.degraded && (json.data || []).length >= 24));
        }
      } catch {
      ...
      async (entries) => {
        if (entries[0].isIntersecting && hasMore && !loading) {
          ...
          try {
            ...
            const response = await fetch(`/api/products?limit=24&store=${encodeURIComponent(store.slug)}&page=${nextPage}`);
            const json = await response.json();
            const more = json.success ? (json.data || []) : [];
            if (more.length > 0) {
              setAllProducts((prev) => {
                const existing = new Set(prev.map((product) => product.id || product.slug));
                return [...prev, ...more.filter((product) => !existing.has(product.id || product.slug))];
              });
              setPage(nextPage);
              setCanLoadMore(Boolean(json.degraded && more.length >= 24));
            } else {
  ...
  return (
    <div className="min-h-screen bg-zinc-950">
      <Header />
      <main>
        <div className={`h-24 bg-gradient-to-r ${logo.gradient} opacity-15 sm:h-36 lg:h-48`} />
        ...
        <PageControls
          ...
          className="pt-2"
        />
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6 flex flex-col items-start gap-4 sm:mb-8 sm:flex-row sm:items-end sm:gap-6">
          <div className={`flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-2xl border-4 border-zinc-950 bg-gradient-to-br ${logo.gradient} shadow-2xl shadow-black/40 sm:h-24 sm:w-24`}>
            <span className="text-lg font-bold uppercase text-white sm:text-2xl">{logo.initials}</span>
          </div>
          <div className="min-w-0 flex-1">
            <Badge variant={store.type === 'farmacia' ? 'info' : 'success'} size="sm" className="mb-1.5">
              {store.type === 'farmacia' ? 'Farmacia' : 'Supermercado'}
            </Badge>
            <h1 className="text-xl font-bold text-zinc-100 sm:text-2xl lg:text-3xl">{store.name}</h1>
        ...
        <div className="mb-6 grid grid-cols-2 gap-2.5 sm:mb-8 sm:grid-cols-4">
          {[
            { icon: Package, label: 'Productos', value: displayedCount },
            ...
            <motion.div key={stat.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className="rounded-xl border border-zinc-800 bg-zinc-900/80 p-3 text-center sm:p-4">
              <stat.icon size={18} className="mx-auto mb-1.5 text-zinc-500" />
              <div className="text-base font-bold text-zinc-100 sm:text-lg">{stat.value}</div>
        ...
        <section>
          <div className="mb-4 flex items-center justify-between sm:mb-6">
            <h2 className="text-lg font-bold text-zinc-100 sm:text-xl">Productos en {store.name}</h2>
            <span className="text-sm text-zinc-500">{displayedCount} productos cargados</span>
          </div>
          {displayed.length > 0 ? (
            ...
            <div className="rounded-2xl border border-dashed border-zinc-800 bg-zinc-900/40 p-8 text-center">
              ...
              <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-zinc-500">
                No hay productos con precio e imagen confiable de {store.name} en este momento. Para mantener la comparacion correcta, no mezclamos productos de otros comercios en esta pagina.
              </p>
